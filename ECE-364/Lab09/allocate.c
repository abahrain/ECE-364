...
void *multialloc(size_t s, int32_t d, ...)
{
        ...
        int32_t max,                /* size of array to be declared */
        ...
        **s1, *t, *tree;        /* base pointer to beginning of first array */
        ...
        int32_t *d1;                /* dimension list */

        va_start(ap,d);
        d1 = (int32_t *) mget_spc(d,sizeof(int));
        ...
        max *= (int32_t)s * (*q);        /* grab actual array memory */
        r[0] = (char *)mget_spc(max,sizeof(char));

        /*
         * r is now set to point32_t to the beginning of each array so that we can
